 ...
 * 
 * 1. non-transactional: ignores commits, updates are permanent.
 * no rollback. a cassandra incrementing state would be like this
 *
 * 2. repeat-transactional:
 * idempotent as long as all batches for a txid are identical
 *
 * 3. opaque-transactional: the most general kind of state. updates are always done based on the
 * previous version of the value if the current commit = latest stored commit Idempotent
 *          even if the batch for a txid can change.
 * 
